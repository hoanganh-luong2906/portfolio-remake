...
function wrapInline(
  ...
  const toggled = selected.startsWith(before) && selected.endsWith(after)
...
function affectedLines(value: string, ss: number, se: number): [number, number] {
...
function toggleLinePrefix(
  ...
  return {
    ...
    selStart: ss + (allHave ? -Math.min(prefix.length, ss - lStart) : prefix.length),
    selEnd: se + delta,
  }
}
...
export function applyFormat(
  ...
  switch (cmd) {
    case 'bold':      return wrapInline(value, ss, se, '**', '**', 'bold text')
    case 'italic':    return wrapInline(value, ss, se, '*', '*', 'italic text')
    case 'code':      return wrapInline(value, ss, se, '`', '`', 'code')
    case 'link': {
    ...
    case 'h1': return toggleLinePrefix(value, ss, se, '# ')
    case 'h2': return toggleLinePrefix(value, ss, se, '## ')
    case 'h3': return toggleLinePrefix(value, ss, se, '### ')
    case 'quote': return toggleLinePrefix(value, ss, se, '> ')
    case 'ul':    return toggleLinePrefix(value, ss, se, '- ')
    case 'ol':    return toggleLinePrefix(value, ss, se, '1. ')
    case 'codeblock': {
      const inner = value.slice(ss, se) || 'code here'
      const insertion = `\`\`\`tsx\n${inner}\n\`\`\``
      const newValue = value.slice(0, ss) + insertion + value.slice(se)
      return { value: newValue, selStart: ss + 7, selEnd: ss + 7 + inner.length }
    }
    case 'hr': {
      ...
      return { value: newValue, selStart: ss + insertion.length, selEnd: ss + insertion.length }
    }
  }
}
...
export function smartEnter(value: string, ss: number): Result | null {
  ...
  if (olm) {
    ...
    return { value: newValue, selStart: ss + next.length, selEnd: ss + next.length }
  }
