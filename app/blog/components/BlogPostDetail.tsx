    ...
    <>
      ...
      <section className="shell pt-[160px] pb-[60px]">
        <div className="reveal mb-[60px]">
          <Link href="/blog" className="mono text-fg-muted inline-flex items-center gap-2">
        ...
        <div className="reveal max-w-[880px]">
          ...
          <p className="body mt-8 text-xl leading-[1.7] max-w-[720px]">{post.excerpt}</p>
          ...
          <div className="flex items-center gap-3.5 mt-10 pt-8 border-t border-line">
            ...
            <div>
              ...
              <div className="text-xs text-fg-muted">Front-end Developer · Ho Chi Minh City</div>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="reveal grid grid-cols-2 gap-4">
          {prev ? (
            <Link href={`/blog/${prev.slug}`} className="glass block p-7">
              <div className="mono text-fg-muted mb-3 text-[11px]">← PREVIOUS</div>
              <div className="text-lg font-medium tracking-[-0.005em]">{prev.title}</div>
            </Link>
          ) : (
            <div />
          )}
          {next ? (
            <Link href={`/blog/${next.slug}`} className="glass block p-7 text-right">
              <div className="mono text-fg-muted mb-3 text-[11px]">NEXT →</div>
              <div className="text-lg font-medium tracking-[-0.005em]">{next.title}</div>
            </Link>
          ) : (
            <div />
          )}
